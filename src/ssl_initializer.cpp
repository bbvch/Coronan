...
namespace {
    ...
    constexpr auto create_NetSSL_context = []() {
        return Context::Ptr{new Context{Context::TLS_CLIENT_USE, private_key_file,
                                      certificate_file, ca_location,
                                      verification_mode, verification_depth,
                                      load_default_cas, cipher_list}};
    };
#endif // WIN32
} // namespace
...
SSLInitializer::initialize_with_accept_certificate_handler()
{
  ...
  auto ssl_initializer = std::make_unique<SSLInitializer>(cert_handler, create_NetSSL_context());
